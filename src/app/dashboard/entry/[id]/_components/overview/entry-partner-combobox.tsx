...
import type { EntryCreateOptionDto } from "@/app/dashboard/entry/_components/types/entry-dashboard-types";
...
interface EntryPartnerComboboxProps {
  id: string;
  label: string;
  value: number;
  ...
  disabled?: boolean;
  invalid?: boolean;
  onValueChange: (option: EntryCreateOptionDto) => void;
  onSearch: (term: string) => Promise<EntryCreateOptionDto[]>;
}

const SEARCH_DEBOUNCE_MS = 400;

export function EntryPartnerCombobox({
  id,
  label,
  value,
  options,
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        setExtraOptions((current) => {
          ...
          return [
            ...current,
            ...results.filter((item) => !knownIds.has(item.id)),
          ];
        });
      } catch {
        // Search actions return an empty list on service failures.
      } finally {
