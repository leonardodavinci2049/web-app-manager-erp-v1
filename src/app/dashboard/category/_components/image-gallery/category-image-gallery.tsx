...
import {
  ...
  setPrimaryCategoryImageAction,
  uploadCategoryImageAction,
} from "../../_actions/category-image-gallery-actions";
import {
  CATEGORY_GALLERY_ACCEPT,
...
export function CategoryImageGallery({
  images,
  totalImages,
  categoryName,
  categoryId,
  ...
  const availableSlots = Math.max(0, CATEGORY_GALLERY_LIMIT - totalImages);
  ...
  const uploadFiles = useCallback(
    async (fileList: FileList | File[]) => {
      ...
      const failures: string[] = files
        ...
        .map(
          (file) => `${file.name}: limite de ${CATEGORY_GALLERY_LIMIT} imagens`,
        );
    ...
    [availableSlots, isBusy, categoryId, refresh],
  );
  ...
  const handleSetPrimary = async () => {
    ...
    try {
      const result = await setPrimaryCategoryImageAction(
        categoryId,
        primaryImageId,
      );
      if (!result.success) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await deleteCategoryImageAction(categoryId, deleteImageId);
      if (!result.success) {
        toast.error(result.error);
        setLiveMessage(result.error);
        return;
  ...
  return (
    <section
      ...
      aria-label={`Galeria de imagens de ${categoryName}`}
    >
      {loadError && (
        <div
          role="alert"
      ...
      <Card className="overflow-hidden py-0">
        <CardContent className="p-0">
          {selectedImage ? (
            <button
              type="button"
              className="group relative block aspect-[3/1] w-full cursor-zoom-in bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-inset"
            ...
            >
              <Image
                ...
                alt={`${categoryName} — ${selectedImage.originalName}`}
                fill
                priority
            ...
            <div className="relative flex aspect-[3/1] flex-col items-center justify-center gap-3 bg-muted p-6 text-center text-muted-foreground">
              <Image
                src={DEFAULT_CATEGORY_IMAGE_URL}
              ...
              <p className="relative text-xs">
                Envie a primeira imagem da categoria.
              </p>
            </div>
          )}
        </CardContent>
      ...
      <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
        {images.map((image, index) => (
          <div
            key={image.id}
            className="relative aspect-[3/1] min-h-11 overflow-hidden rounded-lg border bg-muted"
          >
            <button
              type="button"
        ...
        {availableSlots > 0 && (
          <label
            className={`relative flex aspect-[3/1] min-h-11 cursor-pointer flex-col items-center justify-center gap-1 overflow-hidden rounded-lg border-2 border-dashed bg-muted p-2 text-center text-xs text-muted-foreground transition-colors focus-within:ring-2 focus-within:ring-ring focus-within:ring-inset ${isDragOver ? "border-primary bg-primary/10" : "border-border"}`}
            onDragOver={(event) => {
              event.preventDefault();
              setIsDragOver(true);
            }}
          ...
          >
            <input
              type="file"
              multiple
              accept={CATEGORY_GALLERY_ACCEPT}
              className="sr-only"
              disabled={isBusy}
          ...
          <div className="relative min-h-0 flex-1 bg-black">
            {zoomedImageIndex >= 0 && images[zoomedImageIndex] && (
              <Image
                ...
                alt={`${categoryName} — ${images[zoomedImageIndex].originalName}`}
                fill
                priority
                className="object-contain"
                sizes="100vw"
