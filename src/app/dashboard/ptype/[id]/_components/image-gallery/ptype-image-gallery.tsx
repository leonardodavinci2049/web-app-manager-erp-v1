...
import {
  deletePtypeImageAction,
  setPrimaryPtypeImageAction,
  uploadPtypeImageAction,
} from "../../_actions/ptype-image-gallery-actions";
import {
  DEFAULT_PTYPE_IMAGE_URL,
  PTYPE_GALLERY_ACCEPT,
...
interface PtypeImageGalleryProps {
...
export function PtypeImageGallery({
  images,
  totalImages,
  ptypeName,
  ptypeId,
  ...
  const availableSlots = Math.max(0, PTYPE_GALLERY_LIMIT - totalImages);
  ...
  const uploadFiles = useCallback(
    async (fileList: FileList | File[]) => {
      ...
      const failures: string[] = files
        ...
        .map(
          (file) => `${file.name}: limite de ${PTYPE_GALLERY_LIMIT} imagens`,
        );
      ...
      try {
        for (const file of candidates) {
          ...
          formData.set("ptypeId", ptypeId.toString());
          const result = await uploadPtypeImageAction(formData);
          if (result.success) {
            uploadedCount += 1;
    ...
    [availableSlots, isBusy, ptypeId, refresh],
  );
  ...
  const handleSetPrimary = async () => {
    ...
    try {
      const result = await setPrimaryPtypeImageAction(ptypeId, primaryImageId);
      if (!result.success) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await deletePtypeImageAction(ptypeId, deleteImageId);
      if (!result.success) {
        toast.error(result.error);
        setLiveMessage(result.error);
        return;
  ...
  return (
    ...
    >
          ...
          {selectedImage ? (
            ...
            >
              <Image
                ...
                alt={`${ptypeName} — ${selectedImage.originalName}`}
                fill
                priority
            ...
            <div className="relative flex aspect-square flex-col items-center justify-center gap-3 bg-muted p-6 text-center text-muted-foreground">
              <Image
                src={DEFAULT_PTYPE_IMAGE_URL}
                alt="Tipo de produto sem imagem"
                fill
                priority
              ...
              <p className="relative text-xs">
                Envie a primeira imagem do tipo de produto.
              </p>
            </div>
          )}
        </CardContent>
      ...
      <p className="text-xs text-muted-foreground">
        JPEG, PNG, GIF ou WebP. Até 10 MB por arquivo e {PTYPE_GALLERY_LIMIT}{" "}
        imagens.
      </p>
      ...
      >
        ...
        >
          <DialogHeader className="border-b px-6 py-4 pr-12">
            <DialogTitle>{ptypeName}</DialogTitle>
          </DialogHeader>
          <div className="relative min-h-0 flex-1 bg-black">
            {zoomedImageIndex >= 0 && images[zoomedImageIndex] && (
              <Image
                ...
                alt={`${ptypeName} — ${images[zoomedImageIndex].originalName}`}
                fill
                priority
