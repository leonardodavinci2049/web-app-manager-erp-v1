...
import { uploadPtypeListingImageAction } from "../_actions/ptype-listing-image-actions";
...
interface PtypeImageUploadProps {
  ptypeId: number;
  viewMode: "grid" | "list";
  compact?: boolean;
}
...
/**
 * Gatilho de upload de imagem do tipo de produto na listagem (Client). Aceita
 ...
 * produtos.
 */
export function PtypeImageUpload({
  ptypeId,
  viewMode,
  compact = false,
}: PtypeImageUploadProps) {
  const router = useRouter();
  const [isUploading, setIsUploading] = useState(false);
  const [isDragOver, setIsDragOver] = useState(false);

  const inputId = `ptype-image-file-${viewMode}-${ptypeId}${compact ? "-compact" : ""}`;
  ...
  const handleImageUpload = useCallback(
    async (files: FileList | null) => {
      ...
      try {
        ...
        formData.append("ptypeId", ptypeId.toString());

        const result = await uploadPtypeListingImageAction(formData);
  ...
  return (
    <button
      type="button"
      aria-label="Enviar imagem do tipo de produto"
