...
import {
  deleteCarrierImageAction,
  setPrimaryCarrierImageAction,
  uploadCarrierImageAction,
} from "../../_actions/carrier-image-gallery-actions";
import {
  CARRIER_GALLERY_ACCEPT,
...
interface CarrierImageGalleryProps {
  images: CarrierGalleryImage[];
  totalImages: number;
  carrierName: string;
  carrierId: number;
...
export function CarrierImageGallery({
  images,
  totalImages,
  carrierName,
  carrierId,
  ...
  const availableSlots = Math.max(0, CARRIER_GALLERY_LIMIT - totalImages);
  ...
  const uploadFiles = useCallback(
    async (fileList: FileList | File[]) => {
      ...
      const failures: string[] = files
        ...
        .map(
          (file) => `${file.name}: limite de ${CARRIER_GALLERY_LIMIT} imagens`,
        );
      ...
      try {
        for (const file of candidates) {
          ...
          formData.set("carrierId", carrierId.toString());
          const result = await uploadCarrierImageAction(formData);
          if (result.success) {
            uploadedCount += 1;
    ...
    [availableSlots, isBusy, carrierId, refresh],
  );
  ...
  const handleSetPrimary = async () => {
    ...
    try {
      const result = await setPrimaryCarrierImageAction(
        carrierId,
        primaryImageId,
      );
      if (!result.success) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await deleteCarrierImageAction(carrierId, deleteImageId);
      if (!result.success) {
        toast.error(result.error);
        setLiveMessage(result.error);
        return;
  ...
  return (
    <section
      ...
      aria-label={`Galeria de imagens de ${carrierName}`}
    >
          ...
          {selectedImage ? (
            ...
            >
              <Image
                ...
                alt={`${carrierName} — ${selectedImage.originalName}`}
                fill
                priority
            ...
            <div className="relative flex aspect-square flex-col items-center justify-center gap-3 bg-muted p-6 text-center text-muted-foreground">
              <Image
                src={DEFAULT_CARRIER_IMAGE_URL}
                alt="Transportadora sem imagem"
                fill
                priority
              ...
              <p className="relative text-xs">
                Envie a primeira imagem da transportadora.
              </p>
            </div>
          )}
        </CardContent>
      ...
      >
        ...
        >
          <DialogHeader className="border-b px-6 py-4 pr-12">
            <DialogTitle>{carrierName}</DialogTitle>
          </DialogHeader>
          <div className="relative min-h-0 flex-1 bg-black">
            {zoomedImageIndex >= 0 && images[zoomedImageIndex] && (
              <Image
                ...
                alt={`${carrierName} — ${images[zoomedImageIndex].originalName}`}
                fill
                priority
