...
import { uploadCarrierListingImageAction } from "../_actions/carrier-listing-image-actions";
...
interface CarrierImageUploadProps {
  carrierId: number;
  viewMode: "grid" | "list";
  compact?: boolean;
}
...
/**
 * Gatilho de upload de imagem da transportadora na listagem (Client). Aceita
 * clique ou arrastar e soltar de um unico arquivo de imagem de ate 2 MB,
 * mantem o indicador de carregamento, exibe mensagens por toast e atualiza a
 * listagem apos o sucesso. Reproduz o estilo do gatilho do catalogo de
 * produtos.
 */
export function CarrierImageUpload({
  carrierId,
  viewMode,
  compact = false,
}: CarrierImageUploadProps) {
  const router = useRouter();
  const [isUploading, setIsUploading] = useState(false);
  const [isDragOver, setIsDragOver] = useState(false);

  const inputId = `carrier-image-file-${viewMode}-${carrierId}${compact ? "-compact" : ""}`;
  ...
  const handleImageUpload = useCallback(
    async (files: FileList | null) => {
      ...
      try {
        ...
        formData.append("carrierId", carrierId.toString());

        const result = await uploadCarrierListingImageAction(formData);
  ...
  return (
    <button
      type="button"
      aria-label="Enviar imagem da transportadora"
