...
import {
  deleteCustomerImageAction,
  setPrimaryCustomerImageAction,
  uploadCustomerImageAction,
} from "../../_actions/customer-image-gallery-actions";
import {
  CUSTOMER_GALLERY_ACCEPT,
...
export function CustomerImageGallery({
  images,
  totalImages,
  customerName,
  customerId,
  ...
  const availableSlots = Math.max(0, CUSTOMER_GALLERY_LIMIT - totalImages);
  ...
  const uploadFiles = useCallback(
    async (fileList: FileList | File[]) => {
      ...
      const failures: string[] = files
        ...
        .map(
          (file) => `${file.name}: limite de ${CUSTOMER_GALLERY_LIMIT} imagens`,
        );
      ...
      try {
        for (const file of candidates) {
          ...
          const result = await uploadCustomerImageAction(formData);
          if (result.success) {
            uploadedCount += 1;
    ...
    [availableSlots, isBusy, customerId, refresh],
  );
  ...
  const handleSetPrimary = async () => {
    ...
    try {
      const result = await setPrimaryCustomerImageAction(
        customerId,
        primaryImageId,
      );
      if (!result.success) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await deleteCustomerImageAction(customerId, deleteImageId);
      if (!result.success) {
        toast.error(result.error);
        setLiveMessage(result.error);
        return;
  ...
  return (
    ...
    >
          ...
          {selectedImage ? (
            ...
            >
              <Image
                ...
                alt={`${customerName} — ${selectedImage.originalName}`}
                fill
                priority
            ...
            <div className="relative flex aspect-square flex-col items-center justify-center gap-3 bg-muted p-6 text-center text-muted-foreground">
              <Image
                src={DEFAULT_CUSTOMER_IMAGE_URL}
                alt="Cliente sem imagem"
                fill
                priority
              ...
              <p className="relative text-xs">
                Envie a primeira imagem do cliente.
              </p>
            </div>
          )}
        </CardContent>
        ...
        {availableSlots > 0 && (
          ...
          >
            <input
              type="file"
              multiple
              accept={CUSTOMER_GALLERY_ACCEPT}
              className="sr-only"
              disabled={isBusy}
      ...
      <p className="text-xs text-muted-foreground">
        JPEG, PNG, GIF ou WebP. Até 10 MB por arquivo e {CUSTOMER_GALLERY_LIMIT}{" "}
        imagens.
      </p>
          ...
          <div className="relative min-h-0 flex-1 bg-black">
            {zoomedImageIndex >= 0 && images[zoomedImageIndex] && (
              <Image
                ...
                alt={`${customerName} — ${images[zoomedImageIndex].originalName}`}
                fill
                priority
