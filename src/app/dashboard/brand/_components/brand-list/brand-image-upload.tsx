...
import { uploadBrandListingImageAction } from "../../_actions/brand-listing-image-actions";
...
/**
 * Gatilho de upload de imagem da marca na listagem (Client). Aceita clique
...
export function BrandImageUpload({
  ...
  const inputId = `brand-image-file-${viewMode}-${brandId}${compact ? "-compact" : ""}`;
  ...
  const handleImageUpload = useCallback(
    async (files: FileList | null) => {
      ...
      try {
        ...
        const result = await uploadBrandListingImageAction(formData);
  ...
  return (
    <button
      type="button"
      aria-label="Enviar imagem da marca"
