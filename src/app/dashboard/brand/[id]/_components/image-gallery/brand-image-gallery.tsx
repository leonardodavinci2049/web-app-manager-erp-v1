...
import {
  ...
  setPrimaryBrandImageAction,
  uploadBrandImageAction,
} from "../../_actions/brand-image-gallery-actions";
import {
  BRAND_GALLERY_ACCEPT,
...
export function BrandImageGallery({
  ...
  const availableSlots = Math.max(0, BRAND_GALLERY_LIMIT - totalImages);
  ...
  const uploadFiles = useCallback(
    async (fileList: FileList | File[]) => {
      ...
      const failures: string[] = files
        ...
        .map(
          (file) => `${file.name}: limite de ${BRAND_GALLERY_LIMIT} imagens`,
        );
      ...
      try {
        for (const file of candidates) {
          ...
          formData.set("brandId", brandId.toString());
          const result = await uploadBrandImageAction(formData);
          if (result.success) {
            uploadedCount += 1;
    ...
    [availableSlots, isBusy, brandId, refresh],
  );
  ...
  const handleSetPrimary = async () => {
    ...
    try {
      const result = await setPrimaryBrandImageAction(brandId, primaryImageId);
      if (!result.success) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await deleteBrandImageAction(brandId, deleteImageId);
      if (!result.success) {
        toast.error(result.error);
        setLiveMessage(result.error);
        return;
  ...
  return (
    <section
      ...
      aria-label={`Galeria de imagens de ${brandName}`}
    >
      {loadError && (
        <div
          role="alert"
          ...
          {selectedImage ? (
            ...
            >
              <Image
                ...
                alt={`${brandName} — ${selectedImage.originalName}`}
                fill
                priority
            ...
            <div className="relative flex aspect-square flex-col items-center justify-center gap-3 bg-muted p-6 text-center text-muted-foreground">
              ...
              <p className="relative text-xs">
                Envie a primeira imagem da marca.
              </p>
            </div>
          )}
        </CardContent>
      ...
      <p className="text-xs text-muted-foreground">
        JPEG, PNG, GIF ou WebP. Até 10 MB por arquivo e {BRAND_GALLERY_LIMIT}{" "}
        imagens.
      </p>
          ...
          <div className="relative min-h-0 flex-1 bg-black">
            {zoomedImageIndex >= 0 && images[zoomedImageIndex] && (
              <Image
                ...
                alt={`${brandName} — ${images[zoomedImageIndex].originalName}`}
                fill
                priority
