# Product Catalog Route Agent Guide

This file complements the repository and dashboard guides for
`src/app/dashboard/catalog`. It governs the `/dashboard/catalog` page, its
loading state, and the integration between route-level data loading and the
catalog UI.
...
`/dashboard/catalog` is the canonical product catalog route. It supports:
...
8. Build detail-page return navigation with
   `buildCatalogReturnTo(searchParams, "/dashboard/catalog")`.
...
- Canonical catalog links must use `/dashboard/catalog`.
...
- Visual or interactive changes: validate `/dashboard/catalog` in the
