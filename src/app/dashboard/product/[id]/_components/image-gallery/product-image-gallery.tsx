...
} from "../../_actions/product-image-gallery-actions";
import {
  DEFAULT_PRODUCT_IMAGE_URL,
  PRODUCT_GALLERY_ACCEPT,
  PRODUCT_GALLERY_ACCEPTED_MIME_TYPES,
  PRODUCT_GALLERY_LIMIT,
  PRODUCT_GALLERY_MAX_FILE_SIZE,
} from "./image-gallery-constants";
import type { ProductGalleryImage } from "./image-gallery-types";
...
export function ProductImageGallery({
  ...
  const availableSlots = Math.max(0, PRODUCT_GALLERY_LIMIT - totalImages);
  ...
  const uploadFiles = useCallback(
    async (fileList: FileList | File[]) => {
      ...
      const failures: string[] = files
        ...
        .map(
          (file) => `${file.name}: limite de ${PRODUCT_GALLERY_LIMIT} imagens`,
        );
    ...
    [availableSlots, isBusy, productId, refresh],
  );
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await deleteProductImageAction(productId, deleteImageId);
      if (!result.success) {
        toast.error(result.error);
        setLiveMessage(result.error);
        return;
  ...
  return (
    ...
    >
          ...
          {selectedImage ? (
            ...
            <div className="relative flex aspect-square flex-col items-center justify-center gap-3 bg-muted p-6 text-center text-muted-foreground">
              ...
              <p className="relative text-xs">
                Envie a primeira imagem do produto.
              </p>
            </div>
          )}
        </CardContent>
      ...
      <p className="text-xs text-muted-foreground">
        JPEG, PNG, GIF ou WebP. Até 10 MB por arquivo e {PRODUCT_GALLERY_LIMIT}{" "}
        imagens.
      </p>
          ...
          <div className="relative min-h-0 flex-1 bg-black">
            {zoomedImageIndex >= 0 && images[zoomedImageIndex] && (
              <Image
                ...
                alt={`${productName} — ${images[zoomedImageIndex].originalName}`}
                fill
                priority
