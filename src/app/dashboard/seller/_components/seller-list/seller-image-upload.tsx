...
import { uploadSellerListingImageAction } from "../../_actions/seller-listing-image-actions";
...
/**
 * Gatilho de upload de imagem do vendedor na listagem (Client). Aceita clique
...
export function SellerImageUpload({
  ...
  const inputId = `seller-image-file-${viewMode}-${sellerId}${compact ? "-compact" : ""}`;
  ...
  const handleImageUpload = useCallback(
    async (files: FileList | null) => {
      ...
      try {
        ...
        const result = await uploadSellerListingImageAction(formData);
  ...
  return (
    <button
      type="button"
      aria-label="Enviar imagem do vendedor"
