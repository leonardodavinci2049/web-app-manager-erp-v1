...
import {
  deleteSupplierImageAction,
  setPrimarySupplierImageAction,
  uploadSupplierImageAction,
} from "../../_actions/supplier-image-gallery-actions";
import {
  DEFAULT_SUPPLIERS_IMAGE_URL,
  SUPPLIERS_GALLERY_ACCEPT,
...
interface SupplierImageGalleryProps {
...
export function SupplierImageGallery({
  images,
  totalImages,
  supplierName,
  supplierId,
  ...
  const availableSlots = Math.max(0, SUPPLIERS_GALLERY_LIMIT - totalImages);
  ...
  const uploadFiles = useCallback(
    async (fileList: FileList | File[]) => {
      ...
      const failures: string[] = files
        ...
        .map(
          (file) =>
            `${file.name}: limite de ${SUPPLIERS_GALLERY_LIMIT} imagens`,
        );
      ...
      try {
        for (const file of candidates) {
          ...
          const result = await uploadSupplierImageAction(formData);
          if (result.success) {
            uploadedCount += 1;
    ...
    [availableSlots, isBusy, supplierId, refresh],
  );
  ...
  const handleSetPrimary = async () => {
    ...
    try {
      const result = await setPrimarySupplierImageAction(
        supplierId,
        primaryImageId,
      );
      if (!result.success) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await deleteSupplierImageAction(supplierId, deleteImageId);
      if (!result.success) {
        toast.error(result.error);
        setLiveMessage(result.error);
        return;
  ...
  return (
    ...
    >
        ...
        <CardContent className="p-0">
          {selectedImage ? (
            ...
            <div className="relative flex aspect-square flex-col items-center justify-center gap-3 bg-muted p-6 text-center text-muted-foreground">
              <Image
                src={DEFAULT_SUPPLIERS_IMAGE_URL}
              ...
              <p className="relative text-xs">
                Envie a primeira imagem do fornecedor.
              </p>
            </div>
          )}
        </CardContent>
        ...
        {availableSlots > 0 && (
          ...
          >
            <input
              type="file"
              multiple
              accept={SUPPLIERS_GALLERY_ACCEPT}
              className="sr-only"
              disabled={isBusy}
      ...
      <p className="text-xs text-muted-foreground">
        JPEG, PNG, GIF ou WebP. Até 10 MB por arquivo e{" "}
        {SUPPLIERS_GALLERY_LIMIT} imagens.
      </p>
