          ...
          {supplierImagePath ? (
            <div className="flex items-start gap-3 rounded-md border p-3">
              <div className="relative size-24 shrink-0 overflow-hidden rounded-md bg-muted sm:size-32">
                <Image
                  src={
                    hasSupplierImageError
                      ? DEFAULT_SUPPLIERS_IMAGE_URL
                      : supplierImagePath
                  }
                  alt="Imagem cadastrada no fornecedor"
                  fill
                  className="object-cover"
                  sizes="(min-width: 640px) 128px, 96px"
                  onError={() => setHasSupplierImageError(true)}
                  unoptimized={!hasSupplierImageError}
                />
              </div>
        ...
        <CardContent>
          {galleryImages.length === 0 ? (
            <p className="text-sm text-muted-foreground italic py-4">
              Nenhuma imagem na galeria do Assets API para este fornecedor
            </p>
          ) : (
            <div className="space-y-3">
              ...
              <p className="flex items-start gap-2 rounded-md border border-amber-500/40 bg-amber-500/10 p-3 text-xs text-amber-700 dark:text-amber-400">
                ...
                específico na API de fornecedores.
              </p>
